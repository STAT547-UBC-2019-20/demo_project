...
```{r fig2, fig.cap="Occurence of Autism by Ethnicity",out.width= "125%"}
df <- read_csv("../data/autism_cleaned.csv")
df %>%
...
```
Here is an example of cross-referenceing Figure\@ref(fig:fig2)
```{r }
sample_df <- df %>% head
sample_df
```
